...
export default function App() {
  ...
  const allowedRoutes = useMemo(
    () => routes.filter((item) => roleCode && item.roles.includes(roleCode)),
    [roleCode],
  );
  ...
  useEffect(() => {
    ...
    function scheduleReconnect() {
      if (closedByEffect || reconnectTimer !== undefined) return;
    ...
    function handleWsMessage(data: WsMessage) {
      ...
      if (data.type === 'projectile_updated') {
        setBattleState((prev) => {
          if (!prev || prev.sessionId !== data.data.sessionId) return prev;
