...
- Handler contract coverage now also reaches login/logout cookie semantics, route registration, battle scenario listing, WebSocket origin policy, cookie-token monitor auth fallback, rejected-origin handshake protection, and successful monitor WebSocket upgrade/unregister lifecycle; `go test -count=1 "-coverprofile=coverage.out" ./internal/handlers` now reports about 83.2% statement coverage for `backend/internal/handlers`.
