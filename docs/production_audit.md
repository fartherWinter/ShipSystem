...
- Handler contract coverage now also reaches login/logout cookie semantics, route registration, battle scenario listing, WebSocket origin policy, cookie-token monitor auth fallback, rejected-origin handshake protection, successful RBAC list/read payloads, ship/location/dispatch happy paths, analytics start/stop success payloads, and successful monitor WebSocket upgrade/unregister lifecycle; `go test -count=1 "-coverprofile=coverage.out" ./internal/handlers` now reports about 86.4% statement coverage for `backend/internal/handlers`.
