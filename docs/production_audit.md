...
- Remaining gap: CI/CD enforcement is still missing, centralized metrics/alerting and backup/restore drills are still missing, schema-level uniqueness for snapshot time still needs an explicit migration decision, and browser E2E coverage is still only partial beyond the current auth/RBAC/dashboard/table/dispatch/live-battle/replay/export/alarm/track/monitor/websocket-consumer/battle-delta/reconnect baseline.
...
- Handler contract coverage now also reaches login/logout cookie semantics, route registration, battle scenario listing, WebSocket origin policy, cookie-token monitor auth fallback, rejected-origin handshake protection, successful RBAC list/read payloads, ship/location/dispatch happy paths, battle session/timeline/report happy paths, handler-side validation and bad-request branches, analytics start/stop success payloads, invalid-JSON request branches, and successful monitor WebSocket upgrade/unregister lifecycle; `go test -count=1 "-coverprofile=coverage.out" ./internal/handlers` now reports about 98.1% statement coverage for `backend/internal/handlers`.
- Browser E2E now has a real Playwright base: `frontend/playwright.config.ts` plus mocked browser flows currently cover successful login-to-dashboard rendering, dashboard aggregate cards/latest alarm rendering, viewer-role redirect away from `/rbac`, super-admin RBAC user/role/menu table rendering, ship table create/edit/delete, dispatch event create/status progression, live battle session start/stop, battle replay session loading/step-forward behavior, battle report export downloads, alarm ACK, track query rendering, monitor page connectivity/simulator start, logout, browser-level WebSocket consumer updates for `dispatch_event_updated`, `ship_location_updated`, `battle_state_updated`, `radar_scan_updated`, `projectile_updated`, and `battle_event_created`, plus a disconnect-to-reconnect recovery regression for the monitor page, giving the repo executable browser-level auth/RBAC/dashboard/table/dispatch/live-battle/replay/export/alarm/track/monitor/websocket/battle-delta/reconnect/logout regression checks via `cd frontend`, then `npm run test:e2e`.
- Browser E2E now also covers failed-login resubmission plus first-load failure and manual retry recovery for the dashboard summary, ships list, alarms list, dispatch list, RBAC data load, tracks ship-list/query flows, battle live scenario load/state refresh, and battle replay detail load paths, plus simulator-start failure recovery on the monitor page, tightening the `loading/error/retry` regression line for core operator-facing screens.
- Browser E2E now also verifies WebSocket upsert/replacement semantics for dispatch rows, latest-per-ship monitor locations, projectile status replacement, battle event replacement, and reconnect-to-receive behavior after a monitor socket reconnect.
- Browser E2E can now be pulled into release scripts explicitly: `python scripts/preflight_check.py --only frontend-e2e` passes on this workspace, and `python scripts/collect_release_evidence.py --skip-migration-status --include-frontend-e2e --output-dir .release-evidence/latest-frontend-e2e` archives a manifest-backed `frontend-e2e` step without requiring a reachable local PostgreSQL instance.
