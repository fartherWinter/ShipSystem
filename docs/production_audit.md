...
- Browser E2E now has a real Playwright base: `frontend/playwright.config.ts` plus mocked browser flows currently cover successful login-to-dashboard rendering, viewer-role redirect away from `/rbac`, ship table create/edit/delete, dispatch event create/status progression, battle replay session loading/step-forward behavior, and logout, giving the repo executable browser-level auth/RBAC/table/dispatch/replay/logout regression checks via `cd frontend && npm run test:e2e`.
