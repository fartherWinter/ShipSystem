...
- Handler contract coverage now also reaches login/logout cookie semantics, route registration, battle scenario listing, WebSocket origin policy, and successful monitor WebSocket upgrade/unregister lifecycle; `go test -count=1 "-coverprofile=coverage.out" ./internal/handlers` now reports about 82.6% statement coverage for `backend/internal/handlers`.
