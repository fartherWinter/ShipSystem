...
- Handler contract coverage now also reaches login/logout cookie semantics, route registration, battle scenario listing, WebSocket origin policy, cookie-token monitor auth fallback, rejected-origin handshake protection, successful RBAC list/read payloads, ship/location/dispatch happy paths, battle session/timeline/report happy paths, analytics start/stop success payloads, invalid-JSON request branches, and successful monitor WebSocket upgrade/unregister lifecycle; `go test -count=1 "-coverprofile=coverage.out" ./internal/handlers` now reports about 89.8% statement coverage for `backend/internal/handlers`.
