...
- Browser E2E now also covers first-load failure and manual retry recovery for the dashboard summary, ships list, alarms list, dispatch list, RBAC data load, tracks query, battle live scenario load, and battle replay detail load paths, plus simulator-start failure recovery on the monitor page, tightening the `loading/error/retry` regression line for core operator-facing screens.
