...
| P1 | 前端缺少浏览器级 E2E | 静态路由和 build 能防结构回归，但不能证明页面交互完整 | 引入 Playwright 覆盖登录、导航、表格、地图、对战回放和无权限跳转 |
