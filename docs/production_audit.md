...
- Browser E2E now also covers first-load failure and manual retry recovery for the dashboard summary, ships list, dispatch list, and tracks query pages, plus simulator-start failure recovery on the monitor page, tightening the `loading/error/retry` regression line for core operator-facing screens.
