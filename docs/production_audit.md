...
- Browser E2E now has a real Playwright base: `frontend/playwright.config.ts` plus mocked browser flows currently cover successful login-to-dashboard rendering, viewer-role redirect away from `/rbac`, ship table create/edit/delete, dispatch event create/status progression, live battle session start/stop, battle replay session loading/step-forward behavior, battle report export downloads, alarm ACK, track query rendering, monitor page connectivity/simulator start, and logout, giving the repo executable browser-level auth/RBAC/table/dispatch/live-battle/replay/export/alarm/track/monitor/logout regression checks via `cd frontend`, then `npm run test:e2e`.
